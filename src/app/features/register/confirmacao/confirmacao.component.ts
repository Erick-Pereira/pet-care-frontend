  ...
  export class ConfirmacaoComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
      ...
      this.subscriptions.push(
        this.dadosPessoaisForm.valueChanges.subscribe(() => this.updateProgress()),
    ...
    updateProgress(): void {
      ...
      const totalCampos = forms.reduce((sum, form) => sum + Object.keys(form.controls).length, 0);
      const camposPreenchidos = forms.reduce((sum, form) => {
        return sum + Object.values(form.controls).filter(control => control.value && control.valid).length;
      }, 0);
